#' Add divisions and conference to df
#'
#' @param df df that has team column
#'
#' @return
#' @export
#'
#' @examples pbp_df %>% add_divisions()
...
add_divisions <- function(df) {
  ...
  df <- df %>%
    dplyr::mutate(
      ...
      conference = dplyr::if_else(
        .data$division %in%
          c("AFC North", "AFC South", "AFC East", "AFC West"), "AFC", "NFC"
      )
    )
